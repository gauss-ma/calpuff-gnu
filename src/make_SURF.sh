...
input_file="./datainput/87*"
sfc_output_file="surf.dat"
ppt_output_file="precip.dat"
...
#H E A D E R:
	...
	#datos de la estacion
		line=0$(head -n1 ${input_file});line=$(echo $line | sed -e 's/+/ /g')
...
#PRECIP.DAT (HEADER)
printf '%s\n1\nProduced by Ramiro A. Espada from GAUSS-ma\nNONE\n' "PRECIP.DAT">${ppt_output_file}
printf '%6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f\n' ${ibyr}  ${ibjdy}  ${ibhr}  ${ieyr}  ${iejdy}  ${iehr}  ${stat_time_offset} ${#input_file[@]} >> ${ppt_output_file}
printf '%6.0f\n' ${stat_id} >> ${ppt_output_file}
...
#${ibyr}  ${ibjdy}  ${ibhr}  0  ${ieyr}  ${iejdy}  ${iehr}  0  ${#input_file[@]}
#${stat_id}
#1.0000
#EOF
...
do
	...
	if [[ $this_date < $(date --date="$ibyr-$ibmo-$ibdy $ibhr:$ibmin:$ibsec" +"%Y-%m-%d %H:%M:%S") ]]; then continue;fi;
	if [[ $this_date > $(date --date="$ieyr-$iemo-$iedy $iehr:$iemin:$iesec" +"%Y-%m-%d %H:%M:%S") ]]; then break; fi;
...
#SURF.DAT
printf '%4.0f %4.0f %3.0f\n' $iyr $ijul $ihr >> ${sfc_output_file}
printf '%6.2f %6.1f %6.0f %6.0f %6.2f %6.0f %6.1f %6.0f \n ' $ws $wd $ceil $sky $temp $rh $pres $pp_code >> ${sfc_output_file}

#PRECIP.DAT
printf '%6.0f %6.0f %6.0f %6.2f\n' $iyr $ijul $ihr $pp_r >> ${ppt_output_file}
